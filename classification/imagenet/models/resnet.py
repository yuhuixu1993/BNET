...
__all__ = [
    'ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet50BNET', 'resnet101',
    'resnet101BNET', 'resnet152', 'resnext50_32x4d', 'resnext101_32x4d',
...
class BottleneckBNET(nn.Module):
   # Bottleneck in torchvision places the stride for downsampling at 3x3 convolution(self.conv2)
   # while original implementation places the stride at the first 1x1 convolution(self.conv1)
   # according to "Deep residual learning for image recognition"https://arxiv.org/abs/1512.03385.
   # This variant is also known as ResNet V1.5 and improves accuracy according to
   ...
   def __init__(self,
                ...
                dilation=1,
                norm_layer=None):
      super(BottleneckBNET, self).__init__()
      if norm_layer is None:
         norm_layer = nn.BatchNorm2d
      width = int(planes * (base_width / 64.)) * groups
      # Both self.conv2 and self.downsample layers downsample the input when stride != 1
      ...
      self.bn2 = norm_layer(width)
      self.conv3 = conv1x1(width, planes * self.expansion)
      self.bnet3 = BNET2d(planes * self.expansion)
      self.relu = nn.ReLU(inplace=True)
      self.downsample = downsample
      self.stride = stride
...
class ResNet(nn.Module):
   ...
   def __init__(self,
      ...
      for m in self.modules():
         ...
         if isinstance(m, BNET2d):
            assert m.weight is not None
            assert m.bias is not None
            nn.init.constant_(m.bnconv.weight, 0)
...
def resnet50BNET(pretrained=False, progress=True, **kwargs):
   r"""ResNet-50 model from
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
   return _resnet('resnet50', BottleneckBNET, [3, 4, 6, 3], pretrained,
                  progress, **kwargs)
...
def resnet101BNET(pretrained=False, progress=True, **kwargs):
   r"""ResNet-101 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
   return _resnet('resnet101', BottleneckBNET, [3, 4, 23, 3], pretrained,
                  progress, **kwargs)
...
def resnext101_32x4d(pretrained=False, progress=True, **kwargs):
   ...
   kwargs['groups'] = 32
   kwargs['width_per_group'] = 4
   return _resnet('resnext101_32x4d', Bottleneck, [3, 4, 23, 3], pretrained,
                  progress, **kwargs)
